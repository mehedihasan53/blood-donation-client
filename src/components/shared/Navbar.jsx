          ...
          <div className="flex items-center space-x-3">
            ...
            {user ? (
              <div className="relative user-dropdown">
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-white/95 dark:bg-slate-700/95 backdrop-blur-xl rounded-2xl border border-gray-200/50 dark:border-slate-600/50 py-3 z-50 animate-fade-in shadow-lg">
                    <Link to="/dashboard" className="flex items-center px-5 py-3 text-gray-700 dark:text-slate-200 hover:bg-white/20 dark:hover:bg-white/10 hover:text-red-600 transition-all duration-300 mx-2 rounded-xl backdrop-blur-sm" onClick={() => setIsDropdownOpen(false)}>
                      <FaHome className="mr-4 text-red-500 w-4 h-4" />
                      <span className="font-medium">Dashboard</span>
                    </Link>
                    {role === "admin" && (
                      <Link to="/dashboard/all-users" className="flex items-center px-5 py-3 text-gray-700 dark:text-slate-200 hover:bg-white/20 dark:hover:bg-white/10 hover:text-red-600 transition-all duration-300 mx-2 rounded-xl backdrop-blur-sm" onClick={() => setIsDropdownOpen(false)}>
                        <FaUser className="mr-4 text-red-500 w-4 h-4" />
                        <span className="font-medium">Manage Users</span>
                      </Link>
                    )}
                    <hr className="my-2 border-gray-200/50 dark:border-slate-600/50 mx-2" />
                    <button onClick={handleLogout} className="flex items-center w-full px-5 py-3 text-gray-700 dark:text-slate-200 hover:bg-white/20 dark:hover:bg-white/10 hover:text-red-600 text-left transition-all duration-300 mx-2 rounded-xl backdrop-blur-sm">
              ...
              <div className="hidden lg:flex items-center space-x-4">
                <Link to="/login" className="text-gray-700 dark:text-slate-200 hover:text-red-600 font-semibold px-5 py-3 rounded-xl transition-all duration-300 hover:bg-white/20 dark:hover:bg-white/10 backdrop-blur-sm">Login</Link>
                <Link to="/register" className="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">Join Now</Link>
              </div>
            )}

            <button onClick={toggleMenu} className="lg:hidden text-gray-700 dark:text-slate-200 hover:bg-white/20 dark:hover:bg-white/10 p-3 rounded-xl transition-all duration-300 focus:outline-none backdrop-blur-sm">
            ...
            <div className="flex flex-col space-y-3 px-4">
              {user && (
                <div className="flex items-center p-5 mb-4 bg-gray-50/60 dark:bg-slate-600/40 backdrop-blur-sm rounded-2xl border border-gray-200/40 dark:border-slate-500/40">
                  <img src={user.photoURL} className="w-14 h-14 rounded-full border-3 border-red-500 mr-4 shadow-lg" alt="user" />
                  <div>
                    <p className="font-bold text-gray-900 dark:text-white text-lg">{user.displayName}</p>
                    <p className="text-sm text-red-600 dark:text-red-300 font-semibold capitalize bg-red-100/50 dark:bg-red-900/30 px-3 py-1 rounded-lg inline-block mt-1">{role}</p>
                  </div>
                </div>
              )}
              ...
              <button onClick={handleContactClick} className={`${mobileLinkStyle} w-full text-left flex items-center gap-2`}>
                Contact
              </button>
              ...
              {user ? (
                <>
                  ...
                  <Link to="/dashboard" className={`${mobileLinkStyle} bg-white/30 dark:bg-white/10 text-red-700 dark:text-red-300 font-semibold`} onClick={toggleMenu}>
                    <span className="flex items-center"><FaHome className="mr-3 w-4 h-4" /> Dashboard</span>
                  </Link>
                  {role === "admin" && (
                    <Link to="/dashboard/all-users" className={mobileLinkStyle} onClick={toggleMenu}>
                      <span className="flex items-center"><FaUser className="mr-3 w-4 h-4" /> Manage Users</span>
                    </Link>
                  )}
                  <button onClick={handleLogout} className={`${mobileLinkStyle} text-red-600 dark:text-red-300 w-full text-left font-semibold`}>
                    <span className="flex items-center"><FaSignOutAlt className="mr-3 w-4 h-4" /> Logout</span>
                  </button>
                </>
              ) : (
                <div className="pt-4 border-t border-gray-200/50 dark:border-slate-600/50 flex flex-col space-y-4 mt-4">
                  <Link to="/login" className="block text-center text-gray-700 dark:text-slate-200 py-4 font-semibold hover:bg-white/20 dark:hover:bg-white/10 rounded-xl transition-all duration-300" onClick={toggleMenu}>Login</Link>
                  <Link to="/register" className="block text-center bg-gradient-to-r from-red-500 to-red-600 text-white py-4 rounded-xl font-bold shadow-lg" onClick={toggleMenu}>Join as Donor</Link>
                </div>
              )}
            </div>
          </div>
